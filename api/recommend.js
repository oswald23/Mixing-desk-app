...
/** Extract text from a PDF buffer using pdfjs-dist (server-safe) */
async function pdfBufferToText(buf) {
  try {
    ...
    let text = "";
    const maxPages = Math.min(pdfDoc.numPages || 0, 40); // cap for token budget
...
/** Fetch PDF bytes then extract text */
...
export default async function handler(req, res) {
  try {
    ...
    const kb = await fetchPdfText(pdfUrl); // may be empty if fetch/parse fails — that’s OK
    ...
    const model = process.env.OPENAI_MODEL || "gpt-4o"; // keep to valid API model

    const system = `
You are a coaching assistant. Map any scenario to psychopathic trait "dials" (0–10) using the mixing-desk idea.
If a PDF excerpt is provided, use it as context; quote only tiny phrases. Output STRICT JSON only:
{
